...
std::string bookData(){
                std::ifstream userFile("history.txt"); // Replace "your_file.txt" with your file's path
...
void decreaseCount(const std::string key) {
    ...
    if (it != bookRecord.end() && it->second > 0) {
        ...
        std::cout << "Unable to decrease count for key: " << key << std::endl;
    }
}
...
void queryResponse(std::string bookCode){
    ...
    if (it != bookRecord.end()) {
        if( it->second > 0){
            ...
            std::string dataToSend = std::string("ServerH|The requested book ")+bookCode+ " is available in the library.";
            status = sendUDPData(dataToSend, udpMain);
            decreaseCount(bookCode);
            updateBookCountInFile("history.txt", bookCode, newCount);
            
        }else{
            std::string dataToSend = std::string("ServerH|The requested book ")+bookCode+ " is NOT available in the library.";
            status = sendUDPData(dataToSend, udpMain);
        } // Returns true if count > 0, else false
        if(status>0){
            std::cout<<"Server H finished sending the availability status of code "<<bookCode<< " to the Main Server using UDP on port 44585.\n";
        }
    }
...
void startUDPServer(int portNumber) {
    ...
    std::cout<<"Server H is up and running using UDP on port 43585"<<std::endl;
    ...
    // Find the position of the delimiter '|'
            ...
            if (source=="main"){
                std::cout << "Server H received " << data<<  " code from the Main Server." << std::endl;
                queryResponse(data);
...
std::string onBootUp(){
    ...
    std::string dataToSend = std::string("serverH-boot|")+listOfBook;
...
int main() {
    int udpPortNumber=43585;

   
    onBootUp();
    loadDataFromFile("history.txt");
